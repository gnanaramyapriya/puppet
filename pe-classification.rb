...
class PEClassifier
  ...
  def initialize(env)
    ...
    # Current version being upgraded from
    @CURRENT_PE_MAJOR = env['CURRENT_PE_MAJOR']
    @CURRENT_PE_MINOR = env['CURRENT_PE_MINOR']

    # X.Y PE version we are upgrading to
    @PE_LINK_VER = env['PE_LINK_VER']
  ...
  def add_master
    ...
    profile_master.merge!("file_sync_enabled" => @file_sync_enabled) if @file_sync_enabled_specified
  ...
  def ensure_classes_synced
    pe_class = JSON.parse(get_class(@pe_environment, 'puppet_enterprise'))
    attempts = 0
    ...
    until pe_class['parameters'].key?('pcp_broker_host') || attempts >= @classification_attempts do
      ...
      uri = "#{@nc_api_url}/v1/update-classes?environment=#{@pe_environment}"
      begin
  ...
  def run
    ...
    if ! correct_pe_groups?
      ...
      if ! delete_empty_groups
        $stderr.puts caller
        $stderr.puts "ERROR: We could not classify your PE Infrastructure because #{pe_group_names.join(', ')} contain(s) configuration that we do not want to overwrite. For details about what classes and nodes these groups should contain, please refer to http://docs.puppetlabs.com/pe/#{@PE_LINK_VER}/console_classes_groups_preconfigured_groups.html"
